package io.github.statefulbeans.spring.context;
...
/**
 * Extracts a {@code beanName → beanClass} map from a live Spring
 * {@link ApplicationContext}, filtering out infrastructure beans that are
 * not user-authored (framework internals, Spring Boot auto-configuration, etc.).
 */
public class SpringBeanExtractor {
    ...
    /**
     * Package prefixes considered Spring / Jakarta EE infrastructure that
     * should not be subjected to the statefulness check.
     */
    ...
    public Map<String, Class<?>> extractUserBeans(ApplicationContext applicationContext) {
        ...
        Arrays.stream(beanNames).forEach(name -> {
            try {
                ...
                if (isInfrastructureClass(clazz)) {
                    log.debug("Skipping infrastructure bean: {} ({})", name, clazz.getName());
                    return;
                }
        ...
        log.info("Extracted {} user bean(s) from ApplicationContext (total definitions: {}).",
                result.size(), beanNames.length);
        return result;
    }
