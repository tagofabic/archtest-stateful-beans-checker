...
/**
 * JUnit 5 extension that executes the stateful-bean check before any test
 * methods run in the annotated class.
 *
 * <p>Registered automatically when {@link StatefulBeanCheck} is present on
 * the test class (via {@code @ExtendWith}).</p>
 *
 * <p>If the {@code stateful-beans-checker-spring} module is on the classpath
 * and a Spring {@code ApplicationContext} has been loaded into the
 * {@link ExtensionContext} store (e.g. by {@code @SpringBootTest}), the
 * extension delegates to the Spring-aware runner instead of the package scanner.
 * </p>
 */
public class StatefulBeanCheckExtension implements BeforeAllCallback {
    ...
    /**
     * ExtensionContext store key used by the Spring integration module to
     * register the {@code ApplicationContext} bean map so this extension can
     * pick it up without a hard Spring compile-time dependency.
     */
    ...
    private StatefulBeanCheckConfig buildConfig(StatefulBeanCheck annotation, Class<?> testClass) {
        StatefulBeanCheckConfig.Builder builder = StatefulBeanCheckConfig.builder()
    ...
    private AnalysisResult runAnalysis(StatefulBeanChecker checker,
                                       ExtensionContext context,
                                       StatefulBeanCheck annotation) throws IOException {
        // Check if the Spring integration module registered a bean map
        ExtensionContext.Store store = context.getStore(
                ExtensionContext.Namespace.create(STORE_NAMESPACE));
        Object beanMapObj = store.get(BEAN_MAP_KEY);

        if (beanMapObj instanceof java.util.Map<?, ?> rawMap) {
            log.info("Using Spring ApplicationContext bean map for analysis ({} beans).",
                    rawMap.size());
            java.util.Map<String, Class<?>> beanMap =
                    (java.util.Map<String, Class<?>>) rawMap;
        ...
        log.info("Using classpath package scan for analysis. Packages: {}",
