package dev.tagofabic.statefulbeans.core.config;
...
/**
 * Immutable configuration for a stateful-bean check run. Construct via {@link Builder}.
 *
 * <pre>{@code
...
public final class StatefulBeanCheckConfig {
    ...
    /**
     * Fully-qualified annotation names whose presence on a class exempts it from the check (e.g.
     * "org.springframework.web.bind.annotation.RestController" if you intentionally allow mutable
     * state there).
     */
    ...
    /**
     * When {@code true}, the analyser treats Lombok {@code @Data}, {@code @Setter}, and
     * {@code @Singular} on a bean class as violations even if no explicit Java setter method can be
     * found in the compiled bytecode yet (annotation processors run at compile time so the library
     * user's test classpath may already have the generated methods — but this flag provides an
     * explicit signal to check for them).
     */
    private final boolean lombokAware;

    /**
     * When {@code true}, non-final fields that are recognised Spring injection annotations
     * ({@code @Autowired}, {@code @Inject}, {@code @Value}, {@code @Resource}) are <em>not</em>
     * flagged. Defaults to {@code true}.
     */
    private final boolean allowInjectedFields;

    /**
     * When {@code true}, {@code ThreadLocal} fields are not flagged because they provide per-thread
     * isolation. Defaults to {@code true}.
     */
    ...
    private StatefulBeanCheckConfig(Builder builder) {
        ...
        this.excludedPackages =
                Collections.unmodifiableSet(new HashSet<>(builder.excludedPackages));
        this.excludedAnnotations =
                Collections.unmodifiableSet(new HashSet<>(builder.excludedAnnotations));
    ...
    public boolean isLombokAware() {
        return lombokAware;
    }

    public boolean isAllowInjectedFields() {
        return allowInjectedFields;
    }

    public boolean isAllowThreadLocalFields() {
        return allowThreadLocalFields;
    }
    ...
    public static final class Builder {
        ...
        /**
         * Exclude classes carrying any of the given annotation types. Pass fully-qualified
         * annotation class names as strings so the core module does not need the annotations on its
         * compile classpath.
         */
        ...
        /**
         * Control whether fields annotated with Spring/Jakarta injection annotations are exempted.
         * Default {@code true}.
         */
        ...
        /** Control whether {@code ThreadLocal} fields are exempted. Default {@code true}. */
